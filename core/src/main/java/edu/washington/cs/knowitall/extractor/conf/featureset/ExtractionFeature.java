 ...
 * within that range.
 *
 * For example, the feature `return true if arg2 contains token "fish"` would be implemented by
 * having rangeToExamine() return arg2.getRange() and testAtIndex() returning
 * sentence.getToken(index).equalsIgnoreCase("fish");
 *
 * @author Rob
 */
public abstract class ExtractionFeature implements
    ...
    /**
     * Implements the following naive algorithm for locating the head verb within a verb phrase:<br>
     * <br>
     *
     * 1. Start at end of phrase.<br> 2. Work backward until you encounter a
     * posTag.startsWith("V");<br> 3. Return the corresponding token.<br>
     *
     *
     * If exception, throw an IllegalArgumentException if no verb in the relation. If !exception,
     * return null to the client if no verb in the relation.
     *
