...
public class PatternExtractor {
    ...
    public static String
        pp_np1 =
        "(?:<chunk='B-PP'> <chunk='B-NP'> <chunk='I-NP'>* (?:<chunk='B-PP'> <chunk='B-NP'> <chunk='I-NP'>*)*)";
    public static String
        pp_np2 =
        "(?:<chunk='B-PP'>? <chunk='B-NP'> <chunk='I-NP'>* (?:<chunk='B-PP'> <chunk='B-NP'> <chunk='I-NP'>*)*)";
    public static String
        vp1 =
        "(?:(?:<pos='VBD'> | <pos='VB'> | <pos='VBZ'> | <pos='VBP'> | <pos='MD'> | <pos='VBG'> | <pos='VBN'>) <chunk='I-VP'>* <chunk='B-PRT'>?)";
    public static String
        vp2 =
        "(?:(?:<pos='VBD'> | <pos='VB'> | <pos='VBZ'> | <pos='VBP'> | <pos='MD'>) <chunk='I-VP'>* <chunk='B-PRT'>?)";
    ...
    public static String
        introduces_if =
        "(?:<string='if'> | <string='as'> | <string='while'> | <string='so'> | <string='although'> | <string='whether'> | <string='why'> | <string='where'> | <string='because'> | <string='but'>)";
    ...
    private HashMap<String, RegularExpression<ChunkedSentenceToken>>
        compiledPatternMapArg1 =
        new HashMap<String, RegularExpression<ChunkedSentenceToken>>();

    private HashMap<String, String> patternMapArg2 = new HashMap<String, String>();
    private HashMap<String, RegularExpression<ChunkedSentenceToken>>
        compiledPatternMapArg2 =
        new HashMap<String, RegularExpression<ChunkedSentenceToken>>();
    ...
    private void initPatternMap() {
        ...
        patternMapArg1.put("relative_clause", "<>* " + np + " " + comma
                                              + "? (<pos='WDT'> | <pos='WP'> | <string='that'>) "
                                              + vp + " "
                                              + pp + "? "
                                              + np_pp);// +vp+" "+pp_np2+"? "+comma+"? ");
    ...
    public boolean matchesStatement(ChunkedExtraction extr) {
        ...
        if (matches(ChunkedSentenceToken.tokenize(extr.getSentence().getSubSequence(0, length)),
                    "statement",
                    true)) {
            return true;
        }
    ...
    public boolean matchesObj(ChunkedExtraction extr, int current) {
        ...
        if ((matches(ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(0, length)), "obj",
                     true))) {
            return true;
        }
        return false;
    ...
    public boolean matchesVerbConjSimple(ChunkedExtraction extr, int current) {
        if ((matches(
            ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(current,
                                                                        extr.getStart() - current)),
            "verb_conj_simple1", true))) {
            return true;
        }
        return false;
    }
    ...
    public boolean matchesAppositiveClause(ChunkedExtraction extr, int current) {
        ...
        if (matches(ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(i, length)),
                    "app_1", true)
    ...
    public boolean quotesSubj(ChunkedExtraction extr, int i) {
        if (matches(
            ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(i, extr.getStart() - i)),
            "subj_quotes1", true)
            || matches(
            ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(i,
                                                                        extr.getStart() - i)),
    ...
    public boolean matchesNPList(ChunkedExtraction extr) {
        ...
        List<ChunkedSentenceToken>
            tocheck =
            ChunkedSentenceToken.tokenize(extr.getSentence(), new Range(start,
                                                                        length));
